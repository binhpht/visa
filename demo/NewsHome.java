...
public class NewsHome {
	...
	public News merge(News detachedInstance) {
		...
		try {
			News result = (News) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public News findById(NewsId id) {
		log.debug("getting News instance with id: " + id);
		try {
			News instance = (News) sessionFactory.getCurrentSession().get(
					"News", id);
			if (instance == null) {
	...
	public List findByExample(News instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("News").add(Example.create(instance))
					.list();
